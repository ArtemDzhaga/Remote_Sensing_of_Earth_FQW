...
# Запуск: python -m dem.ingest.opentopography_client (из корня репозитория с активированным venv)
import numpy as np
import rasterio
...
from dem.config import REGIONS, DEFAULT_REGION, DEMTYPE_CHOICES
from dem.io.layout import data_processed, data_raw, resolve_out_dir
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Сырые DEM; пусто = outputs/<дата>/data/raw (VKR_RUN_DATE).",
    )
    parser.add_argument(
        "--processed-dir",
        type=str,
        default="",
        help="Предобработанные DEM; пусто = outputs/<дата>/data/processed.",
    )
    parser.add_argument(
        "--epsg",
        type=int,
    ...
    raw_dir = resolve_out_dir(args.raw_dir, data_raw)
    processed_dir = resolve_out_dir(args.processed_dir, data_processed)
    ensure_dir(raw_dir)
