...
Поддерживаемые спутники (конфиг `dem.config`):
...
from dem.config import DEFAULT_REGION, DEFAULT_SATELLITE, REGIONS, SATELLITES  # noqa: E402
from dem.geo.utils import region_bbox, region_polygon  # noqa: E402
    ...
    *,
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            bbox_wgs84 = region_bbox(region_wgs84)
            bbox = [
            ...
            polygon = region_polygon(region_wgs84)
...
def download_command(args: argparse.Namespace) -> None:
    ...
    out_base = resolve_out_dir(args.out_dir, data_raw)
    run_dir = _make_run_dir(out_base, tag)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python -m dem.ingest.optical_stac download --help\n"
            "  python -m dem.ingest.optical_stac list --help\n\n"
            "Быстрый пример:\n"
            "  python -m dem.ingest.optical_stac download --region sochi_khosta_mzymta_wide "
    ...
    common.add_argument(
        ...
        help="Ключ bbox из dem.config (например sochi_khosta_mzymta_wide).",
    )
    common.add_argument(
        "--satellite",
        type=str,
    ...
    common.add_argument(
        ...
        type=str,
        default="",
        help="Базовая папка для run/scene; пусто = outputs/<дата>/data/raw (VKR_RUN_DATE).",
    )
    common.add_argument(
        "--with-nir",
        action="store_true",
