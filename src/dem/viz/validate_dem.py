...
from dem.io.layout import quality_report_dir, resolve_out_dir  # noqa: E402
...
def dem_stats(path: Path):
    ...
    with rasterio.open(path) as src:
        ...
        meta = {
            ...
            "source_nan_frac": float(np.mean(np.isnan(arr_float))),
            "region_applied": False,
            "region_masked_frac": 0.0,
        }
...
def _mask_outside_polygon(
    ...
    """Маскирует пиксели вне полигона региона."""
    from rasterio.features import geometry_mask
    from rasterio.warp import transform_geom

    poly_in_raster_crs = transform_geom("EPSG:4326", raster_crs, polygon)

    mask = geometry_mask(
        [poly_in_raster_crs],
        out_shape=arr.shape,
        transform=transform,
        invert=True,  # True внутри полигона
        all_touched=False,
    )
    out = arr.copy()
    out[~mask] = np.nan
...
def plot_dem_map(arr: np.ndarray, out_path: Path, bounds, title: str = "DEM") -> None:
    """Карта высот (2D), сохранение в PNG."""
    fig, ax = plt.subplots(1, 1, figsize=(10, 8))
    vmin, vmax = np.nanpercentile(arr, [2, 98])
    extent = (bounds.left, bounds.right, bounds.bottom, bounds.top)
    im = ax.imshow(arr, cmap="terrain", vmin=vmin, vmax=vmax, extent=extent)
    ax.set_title(title)
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
...
def plot_dem_3d(
    ...
    # Локальная система координат улучшает численную устойчивость при больших смещениях.
    xx = xx - float(np.nanmin(xx))
    yy = yy - float(np.nanmin(yy))
    zz = arr_s
...
def write_report(
    ...
    lines = [
        ...
        f"- **Разрешение:** {meta.get('pixel_size_x', float('nan')):.3f} × {meta.get('pixel_size_y', float('nan')):.3f} м/пиксель",
        f"- **Границы (bounds):** {meta['bounds']}",
        f"- **NoData в источнике:** {meta['nodata']}",
        f"- **NaN/NoData в исходном GeoTIFF:** {meta.get('source_nan_frac', nan_frac):.6%}",
        "",
        "## Статистика высот",
        f"- min: {vmin:.3f} м",
        f"- max: {vmax:.3f} м",
        f"- mean: {mean:.3f} м",
        f"- std: {std:.3f} м",
        f"- доля NaN в отчётной карте: {nan_frac:.6%}",
        "",
        "## Визуализация",
    ]
    if meta.get("region_applied"):
        lines.insert(9, f"- **Скрыто вне полигона региона:** {meta.get('region_masked_frac', 0.0):.6%}")
    ...
    lines.extend(["", "## Вывод"])
    source_nan_frac = float(meta.get("source_nan_frac", nan_frac))
    if source_nan_frac == 0 and meta.get("region_applied") and nan_frac > 0:
        lines.append("- В исходном GeoTIFF пропусков нет; белая область на карте — маска вне полигона региона.")
    elif source_nan_frac == 0:
        lines.append("- Пропусков нет; это проверка полноты растра, а не подтверждение высотной точности.")
    else:
        lines.append(f"- В исходном GeoTIFF обнаружены пропуски ({source_nan_frac:.2%}) — при использовании учитывать маску.")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Каталог отчёта; пусто = outputs/<дата>/quality_report (VKR_RUN_DATE).",
    )
    parser.add_argument(
        "--flat",
        action="store_true",
        help="Сохранять файлы прямо в --out-dir (без отдельной timestamp-папки).",
    )
    ...
    parser.add_argument("--no-3d", action="store_true", help="Не строить 3D PNG/HTML (полезно для headless/терминального запуска).")
    parser.add_argument("--no-html", action="store_true", help="Не генерировать интерактивный HTML (только PNG)")
    parser.add_argument("--region", type=str, default="", help="Ключ региона из config.py для маски полигона (например sochi_khosta_mzymta_small)")
    parser.add_argument("--period-label", type=str, default="", help="Метка периода для имени output-папки (например 2024-06)")
    ...
    out_root = resolve_out_dir(args.out_dir, quality_report_dir)
    out_root.mkdir(parents=True, exist_ok=True)
    base = path.stem
    if args.period_label:
        base = f"{base}_period={args.period_label}"
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = out_root if args.flat else (out_root / f"{base}_{stamp}")
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"Чтение DEM: {path}")
    arr, stats, meta = dem_stats(path)
    if args.region:
        from dem.config import REGIONS  # noqa: E402

        region = REGIONS.get(args.region)
        if region is None:
            print(f"Ошибка: неизвестный регион {args.region}")
            sys.exit(1)
        polygon = region.get("polygon")
        if polygon:
            source_nan_frac = float(meta.get("source_nan_frac", np.mean(np.isnan(arr))))
            arr = _mask_outside_polygon(arr, meta["transform"], polygon, meta["crs"])
            meta["region_applied"] = True
            meta["region_masked_frac"] = float(max(0.0, np.mean(np.isnan(arr)) - source_nan_frac))
            # Пересчёт статистики после маски.
            valid = arr[~np.isnan(arr)]
            if valid.size == 0:
                stats = (float("nan"),) * 5
            else:
                stats = (
                    ...
                    float(np.nanstd(arr)),
                    float(np.mean(np.isnan(arr))),
                )
    vmin, vmax, mean, std, nan_frac = stats
    source_nan_frac = float(meta.get("source_nan_frac", nan_frac))
    if meta.get("region_applied"):
        print(
            f"  min={vmin:.2f} max={vmax:.2f} mean={mean:.2f} std={std:.2f} "
            f"source_NaN%={source_nan_frac:.4%} region_masked%={meta.get('region_masked_frac', 0.0):.4%}"
        )
    else:
    ...
    plot_dem_map(arr, fig_map, meta["bounds"], title=f"DEM: {path.name}")
    if args.no_3d:
        fig_3d = None
        fig_3d_html = None
        print("3D визуализация пропущена (--no-3d).")
